...
type PdfVariant = 'summary' | 'full'

type DeepDive = {
  heading: string
  body: string
  diagram?: { spec: DiagramSpec; caption: string }
  media?: { kind: 'image' | 'video' | 'youtube'; src: string; caption: string }
  code?: { label: string; lang: string; pseudo?: boolean; source: string }
}
...
function Header({ project, onDownload }: { project?: boolean; onDownload: (variant: PdfVariant) => void }) {
  return (
    <header className="site-header">
      ...
      <div className="header-actions">
        <div className="header-pdf-group" role="group" aria-label="PDF 저장">
          ...
          <button className="header-pdf" type="button" onClick={() => onDownload('full')} title="상세 구현과 설계 도면까지 포함한 전체 PDF를 저장합니다">
            전체 PDF <Arrow />
          </button>
        </div>
...
function Home({ onDownload }: { onDownload: (variant: PdfVariant) => void }) {
  return (
    <>
      ...
      <main id="main">
        <section className="hero">
          ...
          <div className="hero-bottom">
            <p>Unity와 Unreal Engine, 자체 엔진으로 AR과 네트워크 VR,<br /><strong>디지털 트윈 클라이언트</strong>를 개발해왔습니다.</p>
        ...
        <section className="featured section">
          <div className="section-heading">
            ...
            <div className="section-heading-side">
              <p>프로젝트에서 담당한 업무와 구현 과정, 확인된 결과를 정리했습니다.</p>
...
function ProjectsPage({ onDownload }: { onDownload: (variant: PdfVariant) => void }) {
    ...
    <>
      ...
      <main id="main" className="page-main">
        <section className="page-intro">
          ...
          <h1>프로젝트 소개</h1>
          <p>회사 프로젝트와 개인 및 팀 프로젝트로 나누어, 각각 맡은 역할과 구현 내용을 정리했습니다.</p>
        </section>
...
function ProjectPage({ project, onDownload }: { project: Project; onDownload: (variant: PdfVariant) => void }) {
    ...
    <>
      ...
      <main id="main" className="project-page">
        ...
        {youtubeEmbedUrl && <section className="project-video section">
          <div className="project-video-heading"><p className="section-index">PROJECT VIDEO</p><p>프로젝트 설명에 앞서 주요 장면을 영상으로 확인하실 수 있습니다. <a className="project-video-link" href={project.youtube} target="_blank" rel="noreferrer">YouTube에서 보기 <Arrow /></a></p></div>
        ...
        {project.images && <section className="project-media section">
          ...
          <div className="project-media-grid">
            {project.images.map((image) => <figure key={image.src}><img src={image.src} alt="" /><figcaption>{image.caption}</figcaption></figure>)}
          </div>
        </section>}
        ...
        {project.deepDive && <section className="deep-dive section">
          ...
          {project.deepDive.map((block, i) => (
            <article className="deep-dive-block" key={block.heading}>
              <p className="section-index">{(i + 1).toString().padStart(2, '0')}</p>
              <div>
                <h3>{block.heading}</h3>
                <p>{block.body}</p>
                ...
                {block.code && <figure className="deep-dive-code">
                  <figcaption><span>{block.code.label}</span><em>{block.code.pseudo ? 'PSEUDOCODE' : block.code.lang}</em></figcaption>
                  <pre><code>{block.code.source}</code></pre>
                </figure>}
              </div>
            </article>
          ))}
...
const experience = [
  ['2025.09 — PRESENT', 'OASIS AIX', 'AI Lab · 연구원', 'Unity Android AR과 Unreal Engine 5 VR 클라이언트 개발'],
  ['2023.12 — 2025.04', 'STANS', '주임 연구원', 'Unity XR과 디지털 트윈, Unreal Engine 5 모바일 AR, Tauri 클라이언트 개발'],
  ['2023.06 — 2023.12', 'DANAM SYSTEMS', '연구원', 'LAND400 Phase3 AS9 & AS10 HUMS 임베디드 소프트웨어 개발'],
  ['2023.03 — 2023.05', 'PEARL ABYSS', 'UI 개발 인턴', '블랙스페이스 엔진에서 붉은사막 UI와 디버깅 도구 구현'],
]
    ...
    <section className="profile section">
      <div className="profile-heading">
        ...
        <div>
          ...
          <p>Unity와 Unreal Engine, 자체 엔진, Babylon.js로 클라이언트를 개발했고 Linux와 C 기반 임베디드 소프트웨어도 개발했습니다.</p>
        </div>
      </div>
      <div className="profile-body">
        <div className="profile-statement">
          <p>Unity와 Unreal Engine, 자체 엔진, Babylon.js 환경에서 AR과 VR, 디지털 트윈, 게임 UI를 구현했습니다.</p>
          <p>Unity AI Inference 추론 속도와 전신 트래킹 동기화, 위치 기반 데이터 로딩, UDP 패킷 복구 등 실제 배포 환경에서 발생한 문제를 해결했습니다.</p>
          <p>현재 홍익대학교 영상·커뮤니케이션대학원에서 VR·AR콘텐츠를 전공하며 실시간 3D 클라이언트 개발 범위를 넓히고 있습니다.</p>
        </div>
...
function Contact() {
  return <section className="contact"><p className="section-index">CONTACT</p><h2>프로젝트나 채용 관련 문의는 이메일로 연락 부탁드립니다.</h2><a href="mailto:cendrillio@naver.com">cendrillio@naver.com <Arrow /></a></section>
}

function PrintPortfolio({ variant }: { variant: PdfVariant }) {
  ...
  return (
    <article className={`print-document${isFull ? ' is-full' : ' is-summary'}`}>
      <header className="print-cover">
        ...
        <h1>REAL-TIME<br /><span>3D CLIENT</span><br />ENGINEER.</h1>
        <p className="print-cover-summary">{isFull
          ? 'Unity와 Unreal Engine, 자체 엔진으로 AR과 VR, 디지털 트윈 클라이언트를 개발해온 김세현의 전체 프로젝트 포트폴리오입니다. 주요 프로젝트는 설계 도면과 핵심 코드를 포함한 상세 구현까지 함께 수록했습니다.'
          : 'Unity와 Unreal Engine, 자체 엔진으로 AR과 VR, 디지털 트윈 클라이언트를 개발해온 김세현의 전체 프로젝트 포트폴리오 요약본입니다. 상세 구현이 포함된 전체본은 웹 포트폴리오에서 받으실 수 있습니다.'}</p>
      ...
      <section className="print-project-index">
        ...
        <h2>전체 프로젝트</h2>
        {isFull && <p className="print-index-note">◆ 표시가 있는 프로젝트는 설계 도면과 핵심 코드를 포함한 상세 구현을 함께 수록했습니다.</p>}
        <div className="print-index-grid">
          {projects.map((project) => <a href={`#project-${project.slug}`} key={project.slug}><span>{project.number}</span>{project.title}{isFull && project.deepDive && <b aria-label="상세 구현 수록">◆</b>}</a>)}
        </div>
      </section>
          ...
          <article className="print-project" id={`project-${project.slug}`} key={project.slug}>
            ...
            {isFull && project.deepDive && <div className="print-deep-dive">
              ...
              {project.deepDive.map((block, blockIndex) => (
                <section className="print-deep-dive-block" key={block.heading}>
                  <h4><em>{(blockIndex + 1).toString().padStart(2, '0')}</em>{block.heading}</h4>
                  <p>{block.body}</p>
                  {block.diagram && <Diagram spec={block.diagram.spec} caption={block.diagram.caption} />}
                  {block.media?.kind === 'image' && <figure className="print-deep-dive-media">
                    <img src={block.media.src} alt="" />
                    <figcaption>{block.media.caption}</figcaption>
                  </figure>}
                  {block.media?.kind === 'youtube' && <p className="print-deep-dive-link">
                    {block.media.caption} <a href={block.media.src}>{block.media.src}</a>
                  </p>}
                  {block.code && <figure className="print-deep-dive-code">
                    <figcaption><span>{block.code.label}</span><em>{block.code.pseudo ? 'PSEUDOCODE' : block.code.lang}</em></figcaption>
                    <pre><code>{block.code.source}</code></pre>
                  </figure>}
                </section>
...
function App() {
  ...
  return <>
    ...
    {printVariant && <PrintPortfolio variant={printVariant} />}
  </>
}
